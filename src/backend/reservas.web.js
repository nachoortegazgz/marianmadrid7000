 ...
 * VERSION: v19.6.1-same-day-dual-pairing
...
function _attachPrimaryServiceGuid(slot, forcedServiceGuid, traceId, ctx) {
    const s = _normalizeSlotShape(slot);
    if (!s) return null;
    const forced = _safeTrim(forcedServiceGuid);
    if (!forced || !_looksLikeGuid(forced)) {
        log.error("_attachPrimaryServiceGuid: invalid forcedServiceGuid", { traceId, ctx, forced });
        return null;
    }
    const { serviceId: ignoredServiceId, ...slotWithoutServiceId } = s;
    const out = { ...slotWithoutServiceId, primaryServiceGuid: forced };
    if (out.slot && typeof out.slot === "object") {
        const { serviceId: ignoredNestedServiceId, ...nestedWithoutServiceId } = out.slot;
        out.slot = { ...nestedWithoutServiceId, primaryServiceGuid: forced };
    }
    return out;
}
...
// =============================================================================
function _readImport2Field(service, fieldId, csvHeader = fieldId) {
    if (!service || typeof service !== "object") return undefined;
    const canonicalValue = service[fieldId];
    return canonicalValue !== undefined && canonicalValue !== null ? canonicalValue : service[csvHeader];
}

function _parseImport2Addons(rawAddons) {
...
function _normalizeImport2Addon(addon, fallbackId) {
    const id = _safeTrim(addon?.id || addon?._id || fallbackId);
    if (!id) return null;
    ...
    const rawQuantity = addon?.bookingsAddonQuantity ?? addon?.nativeAddonQuantity;
    const bookingsAddonQuantity = Number.isFinite(Number(rawQuantity)) && Number(rawQuantity) > 0 ? Number(rawQuantity) : null;
    return {
        id,
...
function _resolveAddonContext(service, requestedAddonIds) {
    ...
    const catalogById = new Map(catalog.map((addon) => [_safeTrim(addon?.id), addon]).filter(([id]) => Boolean(id)));
    ...
    for (const addon of selectedAddons) {
        ...
        if (!_looksLikeGuid(nativeId) || !ACTIVE_NATIVE_ADDON_IDS.has(nativeId)) {
            throw new Error("NATIVE_ADDON_NOT_AUTHORIZED");
        }
...
function _readOptionalImport2ResourceIds(rawStaffDisponible, traceId) {
    const source = Array.isArray(rawStaffDisponible) ? rawStaffDisponible : [];
    const resourceIds = Array.from(new Set(
        source
        .map((value) => _safeTrim(value))
        .filter((value) => _looksLikeGuid(value))
    ));
    const ignoredCount = source.length - resourceIds.length;
    if (ignoredCount > 0) {
        log.warn("Import2 staffDisponible contains non-guid values; ignored for runtime availability", {
            count: ignoredCount,
            traceId,
        });
    }
...
async function _mapServiceImport2ToUX(service, traceId) {
    const primaryServiceGuid = _safeTrim(_readImport2Field(service, "primaryServiceGuid") || service?._id);
    if (!_looksLikeGuid(primaryServiceGuid)) {
        throw new Error("Import2 invalid: primaryServiceGuid missing or not a GUID");
    }
    const rawOculto = _readImport2Field(service, "oculto", "Oculto");
    const isHiddenF2 = rawOculto === true || String(rawOculto).toLowerCase() === "true";
    const secondaryRaw = _safeTrim(_readImport2Field(service, "secondaryServiceGuid"));
    const secondaryServiceGuid = secondaryRaw && _looksLikeGuid(secondaryRaw) ? secondaryRaw : null;
    const rawPermitir = _readImport2Field(service, "permitirCombinar", "PermitirCombinar");
    const permitirCombinar = !isHiddenF2 &&
        (rawPermitir === true || String(rawPermitir).toLowerCase() === "true") &&
        !!secondaryServiceGuid;
    const tiempoFase1 = Number(_readImport2Field(service, "tiempoFase1", "TiempoFase1")) || 0;
    const tiempoExposicion = Number(_readImport2Field(service, "tiempoExposicion", "TiempoExposicion")) || 0;
    const tiempoFase2 = Number(_readImport2Field(service, "tiempoFase2", "TiempoFase2")) || 0;
    const duracionTotal = Number(_readImport2Field(service, "duracionTotal", "DuracionTotal")) || 0;
    const tituloServicio = _readImport2Field(service, "tituloServicio", "TituloServicio") || service?.titulo || "Servicio";
    const precio = Number(_readImport2Field(service, "precio", "Precio")) || 0;
    const slugUrl = _readImport2Field(service, "slugUrl", "SlugUrl") || null;
    const rawImage = _safeTrim(_readImport2Field(service, "imagenPrincipal", "Imagen Principal") || service?.imageUrl || "");
    const imageUrl = rawImage.length > 5 ? rawImage : "";
    const localizacion = _safeTrim(_readImport2Field(service, "localizacion", "Localizacion")) || null;
    const resumenCorto = _safeTrim(_readImport2Field(service, "resumenCorto", "ResumenCorto")) || null;
    const descripcionLarga = _safeTrim(_readImport2Field(service, "descripcionLarga", "DescripcionLarga")) || null;
    const recomendacionProductoRef = _safeTrim(
        _readImport2Field(service, "recomendacionProductoRef") ||
        _readImport2Field(service, "productosRecomendadosRef", "ProductosRecomendadosRef")
    ) || null;
    const recomendacionProductoRef2 = _safeTrim(
        _readImport2Field(service, "recomendacionProductoRef2") ||
        _readImport2Field(service, "productosRecomendadosRef2", "ProductosRecomendadosRef2")
    ) || null;
    const estimatedTotal =
        duracionTotal ||
        (permitirCombinar && secondaryServiceGuid ? tiempoFase1 + tiempoExposicion + tiempoFase2 : tiempoFase1) ||
        30;

    // Import2 staff fields are optional UI hints only. Runtime availability comes from Wix Bookings slots.
    const rawStaffDisponible = _readImport2Field(service, "staffDisponible");
    const staffDisponible = _readOptionalImport2ResourceIds(rawStaffDisponible, traceId);
    ...
    let addons = [];
    const rawAddons = _parseImport2Addons(service?.addons);
    if (rawAddons.length > 0) {
        addons = rawAddons.map((addon, index) => _normalizeImport2Addon(addon, `addon${index + 1}`)).filter(Boolean);
    } else {
        for (let i = 1; i <= 3; i++) {
            const name = service?.[`addon${i}`] || service?.[`Addon${i}`];
            const addonPrice = Number(service?.[`addon${i}Precio`] ?? service?.[`Addon${i}Precio`]) || 0;
            if (name && String(name).trim() !== "") {
                addons.push(_normalizeImport2Addon({ id: `addon${i}`, nombre: String(name).trim(), precio: addonPrice }, `addon${i}`));
            }
        }
    }
    ...
    try {
        ...
        if (isGuid) {
            let res = await withTimeout(
                wixData.query(SERVICIOS_COL).limit(1).eq("primaryServiceGuid", clean).find({ suppressAuth: true }),
                WATCHDOG_TIMEOUT_MS,
                "getServiceBySlugOrId:primaryServiceGuid"
            );
            ...
            if (!service && _looksLikeGuid(clean)) {
                res = await withTimeout(
                    wixData.query(SERVICIOS_COL).limit(1).eq("primaryServiceGuid", clean).find({ suppressAuth: true }),
                    WATCHDOG_TIMEOUT_MS,
        ...
        if (mapped.primaryServiceGuid) _cacheSetBounded(serviceCatalogRAM, mapped.primaryServiceGuid, { data: mapped, timestamp: Date.now() }, CACHE_MAX_SIZE);
...
async function _resolvePrimaryServiceGuidInternal(primaryServiceGuidReq) {
    const raw = _safeTrim(primaryServiceGuidReq);
    if (!raw) return null;
    ...
    if (res?.status === "SUCCESS" && res?.data?.primaryServiceGuid) {
        const sid = _safeTrim(res.data.primaryServiceGuid);
...
// =============================================================================
export async function revalidateExactAvailabilitySlot({ primaryServiceGuid, localStartDate, localEndDate, resourceId, nativeAddonIds = [], traceId }) {
    const activeTraceId = traceId || makeTraceId("exact-slot");
    const resolvedPrimaryServiceGuid = await _resolvePrimaryServiceGuidInternal(primaryServiceGuid);
    ...
    if (!resolvedPrimaryServiceGuid || !start || !end) {
    ...
    try {
        ...
        if (normalizedNativeAddonIds.length > 0) {
            const listPayload = {
                serviceId: String(resolvedPrimaryServiceGuid),
                fromLocalDate: start,
                toLocalDate: end,
            ...
            // FIX: do NOT send resourceTypes to getAvailabilityTimeSlot (causes BAD_REQUEST in practice).
        ...
        const normalized = _attachPrimaryServiceGuid(rawSlot, resolvedPrimaryServiceGuid, activeTraceId, "revalidateExactAvailabilitySlot");
...
// =============================================================================
async function _listTimeSlotsV2({ primaryServiceGuid, fromLocalDate, toLocalDate, resourceIds, nativeAddonIds = [] }, options = {}) {
    ...
    const resolvedPrimaryServiceGuid = await _resolvePrimaryServiceGuidInternal(primaryServiceGuid);
    if (!resolvedPrimaryServiceGuid) return [];
    ...
    const cacheKey = `${String(resolvedPrimaryServiceGuid)}__${resourceKey}__${addonKey}__${fromKey}__${toKey}__ts:${timeSlotsPerDay || 0}`;
    if (!skipCache) {
        ...
        try {
            ...
            const slots = rawSlots
                .map((s) => {
                    ...
                    const fixed = _attachPrimaryServiceGuid(s, resolvedPrimaryServiceGuid, traceId, "_listTimeSlotsV2");
...
// =============================================================================
async function _findNextSlotForServiceInternal(primaryServiceGuid, fromLocalDateTime, requiredResourceId, traceId, sameDayOnly = false) {
    const resolvedPrimaryServiceGuid = await _resolvePrimaryServiceGuidInternal(primaryServiceGuid);
    if (!resolvedPrimaryServiceGuid) return { status: "ERROR", data: null, error: { code: "SERVICE_NOT_FOUND", message: "Service GUID not found" } };
    ...
    for (let i = 0; i <= maxDayOffset; i++) {
        ...
        const slots = await _listTimeSlotsV2({ primaryServiceGuid: resolvedPrimaryServiceGuid, fromLocalDate: dayFrom, toLocalDate: dayTo, resourceIds }, { skipCache: true });
...
export async function _getCertifiedDualSlotsInternal(primaryServiceGuid, resourceId, dateYMD, requestedAddonIds = []) {
    const traceId = makeTraceId("dual");
    if (!primaryServiceGuid || !_isValidMadridYmd(dateYMD)) {
        return { status: "ERROR", data: null, error: { code: "INVALID_PARAMS", message: "primaryServiceGuid and valid Madrid dateYMD are required" } };
    }
    const resolvedPrimaryServiceGuid = await _resolvePrimaryServiceGuidInternal(primaryServiceGuid);
    if (!resolvedPrimaryServiceGuid) {
        return { status: "ERROR", data: null, error: { code: "SERVICE_NOT_FOUND", message: "Service GUID not found" } };
    }
    const serviceRes = await _getServiceBySlugOrIdInternal(resolvedPrimaryServiceGuid, traceId);
    ...
    const slotsF1 = await _listTimeSlotsV2({
        primaryServiceGuid: resolvedPrimaryServiceGuid,
        fromLocalDate,
        toLocalDate,
    ...
    if (!isDual) {
        ...
        for (const s1 of slotsF1 || []) {
            ...
            const forcedSlot = _attachPrimaryServiceGuid({ ...s1 }, resolvedPrimaryServiceGuid, traceId, "single");
    ...
    for (const s1 of slotsF1 || []) {
        ...
        const pairToken = _generateUUID();
        const forcedF1 = _attachPrimaryServiceGuid({ ...s1 }, resolvedPrimaryServiceGuid, traceId, "dual:F1");
        const forcedF2 = _attachPrimaryServiceGuid({ ...s2 }, secondaryServiceGuid, traceId, "dual:F2");
    ...
    if (pairs.length > 0) {
        const cachePromises = pairs.map((slotPair) => {
            ...
            const record = {
                ...
                primaryServiceGuid: String(slotPair.primaryServiceGuid),
...
// =============================================================================
export async function _invalidateCachesInternal(primaryServiceGuid, dateYMD, resourceId, traceId = null) {
    const tId = traceId || makeTraceId("invalidate");
    const resolvedPrimaryServiceGuid = (await _resolvePrimaryServiceGuidInternal(primaryServiceGuid)) || _safeTrim(primaryServiceGuid);
    const ymd = _safeTrim(dateYMD);
    if (!resolvedPrimaryServiceGuid || !_isValidMadridYmd(ymd)) return { ok: true, traceId: tId, skipped: true };
    const prefix = `${String(resolvedPrimaryServiceGuid)}__`;
    ...
    const daysCacheId = `${DAYS_CACHE_VERSION}__${String(resolvedPrimaryServiceGuid)}__${_hashKey(resourceIds)}__${String(yearMonth)}`;
    try {
        await withTimeout(
    ...
    try {
        const res = await withTimeout(
            wixData.query(DUAL_CACHE_COL).eq("primaryServiceGuid", String(resolvedPrimaryServiceGuid)).eq("dateYMD", String(ymd)).limit(100).find({ suppressAuth: true }),
            WATCHDOG_TIMEOUT_MS,
...
export const invalidateCachesInternal = webMethod(Permissions.Admin, async (primaryServiceGuid, dateYMD, resourceId) => {
    ...
    try {
        ...
        const res = await _invalidateCachesInternal(primaryServiceGuid, dateYMD, resourceId, traceId);
...
export const resolvePrimaryServiceGuid = webMethod(Permissions.Anyone, async (primaryServiceGuidReq) => {
    const traceId = makeTraceId("wm-svc-resolve");
    try {
        _rateLimitOrThrow("reservas.resolvePrimaryServiceGuid", _safeTrim(primaryServiceGuidReq) || "anon", traceId);
        const resolved = await _resolvePrimaryServiceGuidInternal(primaryServiceGuidReq);
        if (!resolved) {
...
export const getAvailableDays = webMethod(Permissions.Anyone, async (primaryServiceGuid, resourceId, year, month, addonIds = []) => {
    const traceId = makeTraceId("wm-days");
    try {
        _rateLimitOrThrow("reservas.getAvailableDays", `${_safeTrim(primaryServiceGuid)}|${String(year)}|${String(month)}`, traceId);
        const resolved = await _resolvePrimaryServiceGuidInternal(primaryServiceGuid);
...
export const getCertifiedDualSlots = webMethod(Permissions.Anyone, async (primaryServiceGuid, resourceId, dateYMD, addonIds = []) => {
    const traceId = makeTraceId("wm-dual");
    try {
        _rateLimitOrThrow("reservas.getCertifiedDualSlots", `${_safeTrim(primaryServiceGuid)}|${_safeTrim(dateYMD)}`, traceId);
        return await _getCertifiedDualSlotsInternal(primaryServiceGuid, resourceId, dateYMD, addonIds);
    } catch (err) {
        return { status: "ERROR", data: null, error: _toPublicError(err, "DUAL_SLOTS_FAILED") };
    }
});

export const resolveStaffForSlot = webMethod(Permissions.Anyone, async (primaryServiceGuid, start1, rId, addonIds = [], dualContext = null) => {
    const traceId = makeTraceId("wm-staff");
    try {
        _rateLimitOrThrow("reservas.resolveStaffForSlot", `${_safeTrim(primaryServiceGuid)}|${_safeTrim(start1)}`, traceId);
        const resolvedPrimaryServiceGuid = await _resolvePrimaryServiceGuidInternal(primaryServiceGuid);
        if (!resolvedPrimaryServiceGuid) return { status: "ERROR", data: null, error: { code: "SERVICE_NOT_FOUND", message: "Service GUID not found" } };
        const svcRes = await _getServiceBySlugOrIdInternal(resolvedPrimaryServiceGuid, traceId);
        ...
        const slotF1 = _attachPrimaryServiceGuid(slotF1Raw, resolvedPrimaryServiceGuid, traceId, "resolveStaff:F1Norm");
        let candidateResourceIds = _getResourceIdsFromSlot(slotF1);
        // 2. If Dual, intersect with F2 candidates on the same Madrid visit day.
        let slotF2 = null;
        if (dualContext && dualContext.secondaryServiceGuid && dualContext.start2) {
            ...
            const nextF2 = await _findNextSlotForServiceInternal(
                dualContext.secondaryServiceGuid,
                dualContext.start2,
        ...
        return {
            status: "SUCCESS",
            data: {
                slotF1: _attachPrimaryServiceGuid(slotF1, resolvedPrimaryServiceGuid, traceId, "resolveStaff:F1"),
                slotF2: slotF2 ? _attachPrimaryServiceGuid(slotF2, dualContext.secondaryServiceGuid, traceId, "resolveStaff:F2") : null,
